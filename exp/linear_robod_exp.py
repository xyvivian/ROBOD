...
parser.add_argument('--model', type = str, default= "LinearROBOD")
...
for hparam in model_hparams.trials(50):
    print(hparam)
    for exp_num in range(3):
        torch.cuda.empty_cache()     
        hp_name = str('num_layer-%d epochs-%d lr-%.5f weight_decay-%.5f dropout-%.2f num_models-%d 2'
                           % (hparam.num_layer, 
                              hparam.epochs,
                              hparam.lr,
                              hparam.weight_decay,
        ...
        model = LinearROBOD(lr = hparam.lr,
