...
from networks.robod_net import ROBOD_LinearNet,ROBOD_ConvNet
...
class LinearROBOD(nn.Module):
    def __init__(self, 
                 ...
                 is_masked = False,
                 threshold = 1):
        super(LinearROBOD, self).__init__()
    ...
    def fit(self,train_data):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            for idx in range(total_batches):
                ...
                #print(batch_index.shape, data.shape)
...
class ConvROBOD():
    def __init__(self, 
        ...
        self.input_dim_list, self.masks = self.create_mask(num_layer = self.num_layers,
                                                           num_model = self.num_model,
                                                           input_dim = self.input_dim,
                                                           conv_dim_list = self.conv_dim_list, 
                                                           input_expand = self.input_expand, 
                                                           threshold = threshold)
        if not is_masked:
            self.masks = None
        self.model = ROBOD_ConvNet(input_dim_list = self.input_dim_list, 
                                   device = device,
                                   masks = self.masks,
                                   is_masked = is_masked,
                                   num_models = self.num_model)
    ...
    def create_mask(self,
        ...
        for i in range(num_layer):
            if i == 0:
                largest_dim_list.append(input_dim)
            else: 
                next_input = int(conv_dim_list[-1] * (input_expand **(i-1)))
                largest_dim_list.append(next_input) if next_input < threshold \
                else largest_dim_list.append(threshold)
        ...
        for model in range(num_model):
            ...
            for i in range(num_layer):
                if i == 0:
                    ...
                    next_input = int(conv_dim_list[-1-model]* (input_expand **(i-1)))
                    dim_list.append(next_input) if next_input < threshold \
                    else dim_list.append(threshold)
            total_dims.append(dim_list)
